...
count=mean(x[x!=0])
if(zero>zerothresh){
f1=tryCatch({summary(glm.nb(as.formula(paste("count ~ ", paste(c(covars, testingvar),sep="+")),data=data.frame(sample_specs,count=x),subset=x>0))$coefficients[length(covars)+2,c(1,2,4)]},error = function(x) NA,finally={})
returnmat[taxon,"method"]<-"GLMtrunc"
}else{
f1=tryCatch({summary(glm.nb(as.formula(paste("count ~ ", paste(c(covars, testingvar),sep="+")),data=data.frame(sample_specs,count=x)))$coefficients[length(covars)+2,c(1,2,4)]},error = function(x) NA,finally={})
...
zero=mean(x==0)
count=mean(x[x!=0])
f1=tryCatch({summary(lm(as.formula(paste("count ~ ", paste(c(covars, testingvar),sep="+")),data=data.frame(sample_specs,count=trans.arcsine(x))))$coefficients[length(covars)+2,c(1,2,4)]},error = function(x) NA,finally={})
